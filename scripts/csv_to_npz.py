...
# 如果你的输入帧数为1000，并且输入的fps为50，并且机器人是g1_29dof，那么最后得到的形状为[991, 10, 59]
# 59包括根位置3，根朝向6，关节29，末端执行器位置相对于根节点的位置5*3，根线速度和根角速度6
# 其中根位置的xy 是相对于窗口最后一帧根节点位置、在最后一帧偏航坐标系下的偏移；z 保持世界坐标系高度
# 根节点旋转：窗口最后一帧偏航逆旋转 当前帧根四元数，再转换为 6D “切线 + 法线” 表示（旋转矩阵的第1列和第3列）
# 这里是每十帧去保存一个记录，所以1000帧得到的是991个数据
