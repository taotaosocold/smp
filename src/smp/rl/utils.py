...
def load_denoiser(
  ...
  q_high, feature_dim, window_size, t_q_low, t_q_high)``.  ``t_q_low`` / ``t_q_high``
  are ``None`` for unconditional models."""
  ...
  terrain_dim_raw = cfg.get("terrain_dim")
  terrain_dim = int(terrain_dim_raw) if terrain_dim_raw is not None else None
  ...
  return model, scheduler, q_low, q_high, feature_dim, window_size, t_q_low, t_q_high
...
class MotionFeatureBuffer:
  ...
  def reset(
    ...
    if self.terrain is not None and terrain is not None:
  ...
  def get_terrain(self) -> torch.Tensor | None:
    """Return terrain window ``(num_envs, W, terrain_dim)`` or ``None``."""
    return self.terrain
